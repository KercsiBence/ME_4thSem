...
{
    if ( (pong->ball.x - pong->ball.radius < 50) && ((pong->ball.y+pong->ball.radius>=pong->left_pad.y) && (pong->ball.y-pong->ball.radius)<=pong->left_pad.y+120 ) ) {
        pong->ball.x = pong->ball.radius + 50;
        pong->ball.speed_x *= -1;
    }
    if ( (pong->ball.x + pong->ball.radius > pong->width - 50) && ((pong->ball.y+pong->ball.radius>=pong->right_pad.y) && (pong->ball.y-pong->ball.radius)<=pong->right_pad.y+120 ) ) {
